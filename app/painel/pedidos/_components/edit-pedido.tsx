...
export default function EditPedido({ pedido, produtos }: EditPedidoProps) {
  ...
  // Carregar produtos existentes quando o modal abrir
  useEffect(() => {
    if (open) {
      const produtosExistentes: ProdutoSelecionado[] = pedido.produtos.map(p => ({
        produtoId: p.produto.id,
        nome: p.produto.nome,
        preco: p.produto.preco,
        quantidade: p.quantidade
      }))
      setProdutosSelecionados(produtosExistentes)
    }
  }, [open, pedido.produtos])
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Atualize os dados do cliente e produtos do pedido.
          </DialogDescription>
        </DialogHeader>
